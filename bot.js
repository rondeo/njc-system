...
const devs = ['502509585370841098'];
const db = require('quick.db');
const premium = ['502509585370841098']
...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['!nhelp','N J C Community','N J C Shop - 11 Server'];
    var i = -1;
    var j = 0;
...
if (message.content.startsWith(prefix + 'help')) { /// This is The DMS Code Send The Help In DMS // Code By N J C
    let pages = [`**
        ***__General orders__***
**
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'N J C Community ©\'s translator',
...
client.on('guildCreate', guild => {
  ...
  .setDescription(`**
  New Server Add N J C Community © ?
...
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=435392018693488641&scope=bot&permissions=2146958591')
  .setDescription(`**
  Server Kicked N J C Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
...
const adminprefix = "!hema";//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
...
client.on('message', message => {
if(message.content.includes("<@502509585370841098>")) {
...
client.on("message", (message) => {
    ...
    switch(command) {
        ...
        if (!message.member.hasPermission("MANAGE_CHANNELS")) return message.reply("**ليس لديك صلاحيات ال MANGE_SERVER**")
